...
#define delay(ms) Delay(ms)		///TBD:wait 2 fix
#define get_tick() SysTickUptime	///TBD:wait 2 fix
...
static void	W25Qx_Reset(void)
{
	...
	SPI1_WriteByte(cmd, 2, W25QXXXX_TIMEOUT_VALUE);///TBD:wait 2 fix
	W25Qx_Disable();
...
static uint8_t W25Qx_GetStatus(void)
{
	...
	SPI1_WriteByte(cmd, 1, W25QXXXX_TIMEOUT_VALUE);///TBD:wait 2 fix
	/* Reception of the data */
	SPI1_ReadByte(&status, 1, W25QXXXX_TIMEOUT_VALUE);///TBD:wait 2 fix
	W25Qx_Disable();
...
uint8_t W25Qx_WriteEnable(void)
{
    ...
    if((get_tick() - tickstart) > W25QXXXX_TIMEOUT_VALUE)///gettick 是SysTickUptime，单位是ms
    {        
...
void W25Qx_Read_ID(uint16_t *ID)
{
	...
	W25Qx_Enable();
	/* Send the read ID command */
	SPI1_WriteByte(cmd, 4, W25QXXXX_TIMEOUT_VALUE);///TBD:wait 2 fix
	/* Reception of the data */
	SPI1_ReadByte(idt, 2, W25QXXXX_TIMEOUT_VALUE);///TBD:wait 2 fix
...
uint8_t W25Qx_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  do
  {
		...
		W25Qx_Enable();
    /* Send the command */
    if (SPI1_WriteByte(cmd, 4, W25QXXXX_TIMEOUT_VALUE) != 1)///TBD:wait 2 fix
    {
      return W25Qx_ERROR;
    }
    
    /* Transmission of the data */
    if (SPI1_WriteByte(pData,current_size, W25QXXXX_TIMEOUT_VALUE) != 1)///TBD:wait 2 fix
    {
      return W25Qx_ERROR;
    }
		W25Qx_Disable();
    	/* Wait the end of Flash writing */
		while(W25Qx_GetStatus() == W25Qx_BUSY)///TBD:wait 2 fix
		{
			/* Check for the Timeout */
